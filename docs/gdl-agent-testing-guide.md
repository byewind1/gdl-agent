...
**错误提示**：
```
zsh: command not found: gdl-agent
```
...
```bash
gdl-agent run "创建一个参数化门，宽度A，高度B，厚度50mm，包含门框和门板"
```
